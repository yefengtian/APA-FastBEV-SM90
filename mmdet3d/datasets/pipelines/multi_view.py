...
class MultiViewPipeline:
    ...
    def __call__(self, results):
        ...
        if not self.sequential:
            ...
            assert self.n_times > 0, self.n_times
            assert len(results['img_info']) % self.n_times == 0, \
                f'img info: {len(results["img_info"])}, n_times: {self.n_times}'
