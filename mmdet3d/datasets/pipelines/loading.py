...
@PIPELINES.register_module()
class PrepareImageInputsFish_Fullsize(object):
    def __init__(
        self,
        data_config,
        is_train=False,
    ...
    def img_transform(self, img, post_rot, post_tran, resize, resize_dims,
        ...
        # post-homography transformation
        # 原来是 post_rot *= resize（标量），现在要对 x/y 分别缩放
        ...
        if flip:
            A = post_rot.new_tensor([[-1, 0], [0, 1]])
            b = post_tran.new_tensor([crop[2] - crop[0], 0])
            post_rot = A.matmul(post_rot)
            post_tran = A.matmul(post_tran) + b

        if rotate != 0:
            A = self.get_rot(rotate / 180 * np.pi).to(device=post_rot.device, dtype=post_rot.dtype)
            b = post_tran.new_tensor([crop[2] - crop[0], crop[3] - crop[1]]) / 2
            b = A.matmul(-b) + b
            post_rot = A.matmul(post_rot)
            post_tran = A.matmul(post_tran) + b
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        if cam_names is None:
            # Fallback for datasets that don't provide explicit camera names.
        ...
        for i, cam in enumerate(cam_names):
            ...
            post_tran = torch.zeros(2)
            # img aug
            img_augs = self.sample_augmentation(H=src_img.height, W=src_img.width, flip=flip, scale=scale)
            ...
            post_tran[:2] = post_tran2
            post_rot[:2, :2] = post_rot2
            canvas.append(np.array(src_img).shape)
            imgs.append(self.normalize_img(img))
            ...
            if 'calibs' in results:
                ...
                # Fallback path: build fish-style calibration tuple from lidar2img_aug.
            ...
            dist_list.append(torch.tensor(dist))

        img_tr = torch.stack(imgs)
        cam2ego_tr = torch.stack(cam2ego_list)
        intrin_tr = torch.stack(intrin_list)
        post_rot_tr = torch.stack(post_rot_list)
        post_tran_tr = torch.stack(post_tran_list)
        dist_tr = torch.stack(dist_list)
        results['canvas'] = canvas
        
        return img_tr, cam2ego_tr, intrin_tr, post_rot_tr, post_tran_tr, dist_tr
    def __call__(self, results):
        results['img_inputs'] = self.get_inputs(results)
        return results
...
@PIPELINES.register_module()
class LoadAnnotationsBEVDepthFish(object):
    def __init__(self, bda_aug_conf, is_train=True, occ_cls=7, spe_cls=4):
        self.bda_aug_conf = bda_aug_conf
        self.is_train = is_train
        self.occ_cls = occ_cls
        self.spe_cls = spe_cls
    ...
    def bev_transform(self, corner_points, rotate_angle, scale_ratio, flip_dx,
                      flip_dy, point_type='corner'):
        ...
        if corner_points.shape[0] > 0:
            corner_points = corner_points.reshape(-1, 3)

            corner_points = rot_mat @ corner_points.T
            if point_type == 'corner':
                
                corner_points = corner_points.T.reshape(-1, 4, 3)
            elif point_type == 'wheel_stop':
                corner_points = corner_points.T.reshape(-1, 2, 3)
        
        return corner_points, rot_mat

    def __call__(self, results):
        anno = results['ann_info']
        corner_points = np.array(anno['corner_points'])
        # spe = anno['spe']
        # opy = anno['opy']
        spe = [3 if x == -1 else x for x in anno['spe']]
        spe = [min(x, self.spe_cls - 1) for x in anno['spe']]
        opy = [min(x, self.occ_cls - 1) for x in anno['opy']]
        
        wheel_stop_points = np.array(anno['wheel_stop_points']) if 'wheel_stop_points' in anno else np.array([])
        
        rotate_bda, scale_bda, flip_dx, flip_dy = self.sample_bda_augmentation()

        bda_mat = torch.zeros(4, 4)
        bda_mat[3, 3] = 1
        corner_points, bda_rot = self.bev_transform(corner_points, rotate_bda, scale_bda,
                                               flip_dx, flip_dy, point_type='corner')
        if len(wheel_stop_points) > 0:
            wheel_stop_points, _ = self.bev_transform(wheel_stop_points, rotate_bda, scale_bda,
                                               flip_dx, flip_dy, point_type='wheel_stop')
        bda_mat[:3, :3] = bda_rot

        if len(corner_points) == 0:
            corner_points = torch.zeros(0, 4, 3)
        if len(wheel_stop_points) == 0:
            wheel_stop_points = torch.zeros(0, 2, 3)

        results['corner_points'] = corner_points
        results['spe'] = np.array(spe)
        results['opy'] = np.array(opy)
        results['wheel_stop_points'] = wheel_stop_points

        results['img_inputs'] = (*results['img_inputs'], bda_rot)
