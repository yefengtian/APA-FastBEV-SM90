...
__global__ void group_points_grad_kernel(
  ...
  grad_out += bs_idx * c * npoints * nsample +
              c_idx * npoints * nsample +
              pt_idx * nsample + sample_idx;
  idx += bs_idx * npoints * nsample +
         pt_idx * nsample + sample_idx;

  atomicAdd(grad_points + bs_idx * c * n + c_idx * n + idx[0],
            grad_out[0]);
}
...
__global__ void group_points_kernel(
  ...
  int out_idx = bs_idx * c * npoints * nsample +
                c_idx * npoints * nsample +
                pt_idx * nsample + sample_idx;
...
void group_points_kernel_launcher(
  ...
  // cudaDeviceSynchronize();  // 调试 printf 时可以打开
  err = cudaGetLastError();
  if (cudaSuccess != err) {
    fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
    exit(-1);
